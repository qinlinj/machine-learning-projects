...
\newcommand{\outDate}{Feb. 3, 2023}
\newcommand{\dueDate}{Feb. 10, 2023}
\newcommand{\taNames}{Asmita, Dishani, Yash, Siva, Monica, Neural the Narwhal}
...
\begin{parts}
   ...
   \subpart[1] \sone Refer to Figure \ref{fig:dtree}. Note that $D_{test}$ was not used for training or pruning. When the size of the pruned tree is at \textbf{25 nodes}, what is its accuracy on $D_{test}$ likely to be?
    ...
    \textbf{NOTE:} For subparts (a) and (b), 2 models "behave exactly" when the decision boundary produced by them are same leading to the same performance on all possible test data.
    ...
    \begin{your_solution}[title=Your answer:,height=6cm,width=15cm]
        In a 1-dimensional space, a 1-nearest neighbor classifier is equivalent to a decision tree where each node splits on the midpoint between two consecutive training points. The left child of the node would represent the region to the left of the split, and the right child would represent the region to the right of the split. The tree would be balanced such that each leaf node would correspond to a single training point and its associated label.

This would result in a decision tree that behaves exactly the same as the 1-nearest neighbor classifier, since the same distance metric is used to determine the closest training point, and the same label is assigned to any test point that falls within the same region as a given training point. The decision tree would be a perfect representation of the 1-NN classifier, with the same decision boundary and the same prediction for any test point.
    \end{your_solution}
    ...
    \checkboxchar{$\Box$} \checkedchar{$\blacksquare$}
    \begin{checkboxes}
        \CorrectChoice Points: $\{(-5,5), (5,5)\}$, Labels: $\{0,1\}$
        \choice Points: $\{(0,-9), (0,10)\}$, Labels: $\{0,1\}$
        \choice Points: $\{(3,4), (-3,-4)\}$, Labels: $\{0,1\}$
        \choice Points: $\{(0,0), (0,1), (1,0)\}$, Labels: $\{0,0,1\}$
        \CorrectChoice Points: $\{(0,0), (0,1), (1,0)\}$, Labels: $\{0,1,1\}$
        \CorrectChoice Points: $\{(1,2), (3,4), (5,6)\}$, Labels: $\{0,1,1\}$
        \choice None of the above
    \end{checkboxes}
    }
    \end{subparts}
...
\begin{parts}
    ...
    \begin{checkboxes}
        \CorrectChoice Yes, lowering validation error instead of training error is better because lowering training error will not help generalize our model and may lead to overfitting.
    ...
    This would result in over-fitting to the test set and the true performance of the model on unseen data would be overestimated. The test set is meant to be a representative sample of data to evaluate the model's generalization performance and using it during training would undermine the validity of this evaluation.
    \end{your_solution}
...
\begin{parts}
    ...
    \begin{checkboxes}
        ...
        \CorrectChoice If the whole dataset is shifted away from origin i.e. translated by adding a constant to each feature, then the mistake bound will also increase.
...
\begin{parts}
    ...
    \begin{subparts}
        ...
        [(3.0 * 9.0 + 0.0 - 1.0) * 9.0 + (3.0 * 2.0 + 0.0 - 0.0) * 2.0 + (3.0 * 6.0 + 0.0 - 3.0) * 6.0 + (3.0 * 1.0 + 0.0 - 0.0) * 1.0 + (3.0 * 8.0 + 0.0 - 1.0) * 8.0]
        \end{your_solution}
    \clearpage
    ...
    \begin{subparts}
        ...
        The linear regression parameters that minimize the mean-squared error on the duplicated data may differ from the parameters learned on D1 because the duplicated data can change the distribution of the data and the mean-squared error. 
        Duplicating a subset of the rows in a data set can have a significant impact on the linear regression parameters that minimize the mean-squared error, as it can change the distribution of the data and increase the variance of the data, making the best-fit line less accurate.
        \end{your_solution}
...
\] We are interested in minimizing loss over our training data, so we minimize the average log-cosh loss over all points in $\Dc$. 
    ...
    \begin{subparts}
        ...
        \begin{your_solution}[title=Your answer:,height=4cm,width=15cm]
        $$J(\theta) = \frac{1}{N} \sum_{i=1}^N \log(\cosh(\theta^T x(i) - y(i)))$$
        \end{your_solution}
        ...
        \begin{your_solution}[title=Your answer:,height=6cm,width=15cm]
        $$\frac{\partial J(i)(\theta)}{\partial \theta_j} = \frac{\partial}{\partial \theta_j} \tanh(\theta^T x(i) - y(i)) \cdot x_j$$
        \end{your_solution}
        ...
        $$\nabla_{\theta} J(i)(\theta) = (\frac{\partial}{\partial \theta_1} \tanh(\theta^T x(i) - y(i)) \cdot x_1, $$ 
        $$\frac{\partial}{\partial \theta_2}
        \tanh(\theta^T x(i) - y(i)) \cdot x_2, \dots, \frac{\partial}{\partial \theta_k} \tanh(\theta^T x(i) - y(i)) \cdot x_k)$$
        \end{your_solution}
...
sum_of_gradients = 0
for i = 1 to N
    calculate gradient = gradient theta J(i)(theta)
    sum_of_gradients = sum_of_gradients + gradient
mean_gradient = sum_of_gradients / N
update theta = theta - alpha * mean_gradient
