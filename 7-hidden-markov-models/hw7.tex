...
\newcommand{\dueDate}{Monday, April 10th}
...
$P(A,B,C,D,E) = P(A) \cdot P(B|A,C) \cdot P(C|D) \cdot P(D∣|A) \cdot P(E|A,B,D)$
\end{your_solution}
...
\begin{subparts}
\subpart[3] \sone What is the relation between $\sum_{i=1}^{J}(\alpha_5(s_i)\beta_5(s_i))$ and $P(x_{1:T})$, for $T > 5$? Select only the \textbf{strongest} relation that necessarily holds.
\begin{checkboxes}
    \CorrectChoice $=$
    \choice $>$
    \choice $<$
...
\begin{parts}
    \part[2] Compute $\omega_1(C)$ and $\omega_1(D)$. If your answers involve decimal numbers, please round your answer to \textbf{TWO} decimal places.
    ...
    $\omega_1(C) = \pi_C \cdot A_{C1} = 0.5 \cdot 0.5 = 0.25$
    % YOUR ANSWER 
    \end{your_solution}
    ...
    \begin{your_solution}[title=Work,height=3cm,width=12cm]
    % YOUR ANSWER 
    $\omega_1(D) = \pi_D \cdot A_{D1} = 0.5 \cdot 0.3 = 0.15$
    \end{your_solution}
...
\begin{parts}
    ...
    \part[3] Compute the log-likelihood for the entire validation sequence, ``\texttt{fish eat you}", using $e$ as the log base.
    ...
    \clearpage
    \part
    In the forward algorithm, recall that the entries in $\bm\alpha$ can be computed using the following dynamic programming algorithm:
        \begin{lstlisting}[escapechar=:]
:$\alpha_1(s_j) = \pi_j \cdot A_{s_j,x_1}$: for all :$s_j$:
For :$t = 1, 2, 3, \ldots, T$:
    :$\alpha_t(s_j) = A_{s_j,x_t} \sum_k B_{s_k,s_j} \alpha_{t-1}(s_k)\text{ for all }s_j$:
        \end{lstlisting}
        
% Derive $\log\big(\alpha_1(j)\big)$ in terms of $\log(\pi_j)$ and $\log(A_{jx_1})$
    
\vspace{0.25in}


To implement the forward algorithm in log-space, we can derive $\log\big(\alpha_{t}(s_j)\big)$ in terms of $\log(\alphav_{t-1})$, $\log\Bv$ and $\log \Av$:


$\log\big(\alpha_{t}(s_j)\big)$\\[0.05 in]
$= \log \big(A_{j,x_t} \sum_k B_{kj} \alpha_{t-1}(s_k) \big)$\\[0.05 in]
$= \log(A_{j,x_t}) + \log \big(\sum_k B_{kj} \alpha_{t-1}(s_k)\big)$ \\[0.05 in]
$= $ $\log(A_{j,x_t}) + \log \bigg(\sum_{k=1}^J e^{\log \big(\alpha_{t-1}(k) B_{kj} \big)} \bigg)$\\[0.05 in]
$= \log(A_{j,x_t}) + \log \bigg(\sum_{k=1}^J e^{\log\big(\alpha_{t-1}(k)\big) + \log \big(B_{kj} \big)} \bigg)$\\[0.05 in]
...
\end{lstlisting}
    ...
    % \item Derive $\log\big(\beta_T(j)\big)$

    % $\log\big(\beta_T(j)\big) = \log(1) = 0$
...
A_{:,x_3}^T)\bm\beta_3$
    ...
    % non-vectorized version moved to recitation as of Fall 2022
...
Construct a plot with number of sequences used for training on the x-axis and average log likelihood across all sequences from the English \textbf{train.txt} and the \textbf{validation.txt} on the y-axis (see Section~\ref{forback} for details on computing the log data likelihood for a sequence). Please use \textbf{log-scale} with base $10$ for the x-axis. 
