...
def random_init(shape):
    ...
    return np.random.uniform(-0.1, 0.1, (shape[0], shape[1]))
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        :return: partial derivative of loss with respect to input of
            sigmoid activation
        """
        ds = self.sigmoid_output * (1 - self.sigmoid_output)
        return dz * ds
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        h = self.sigmoid.forward(z1)
        z2 = self.linear2.forward(h)
        y_hat, loss = self.loss.forward(z2, y)
        return y_hat, loss
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        :param y_hat: prediction with shape (num_classes,)
        """
        grad_z2 = self.loss.backward(y, y_hat)
        grad_h = self.linear2.backward(grad_z2)
        grad_z1 = self.sigmoid.backward(grad_h)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for epoch in range(n_epochs):
            ...
            X_tr, y_tr = shuffle(X_tr_original, y_tr_original, epoch)

            for i in range(X_tr.shape[0]):
                x, y = X_tr[i], y_tr[i]

                # Forward pass
                yh, _ = self.forward(x, y)
            ...
            # Record train and test losses after each epoch
