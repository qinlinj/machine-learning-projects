...
\newcommand{\courseName}{10-301/10-601 Introduction to Machine Learning (Spring 2023)}
\newcommand{\hwName}{Homework 5: Neural Networks}
\newcommand{\dueDate}{Friday, March 17 at 11:59 PM}
...
OUT: Sunday, February 26 \\
DUE: \dueDate{} \\ 
TAs: Abhi, Tara, Hang, Ads, Poorvi
}
...
$$\boldsymbol{\beta}=
    ...
    \begin{subparts}
        ...
        \CorrectChoice B) Append a value of 1 to be the first entry of $\zv$.
...
\begin{subparts}
    ...
    $$\frac{\partial \ell}{\partial \hat{y}_i} = -y_i \left(\frac{1}{\hat{y}_i}\right)$$
    \end{your_solution}
    ...
    % YOUR ANSWER
    $$\frac{\partial \ell}{\partial \beta_{kj}} = (\frac{\partial \ell}{\partial b_{k}})z_j$$

    $$\frac{\partial \ell}{\partial \beta} =
    (\frac{\partial \ell}{\partial b})z$$
    ...
    % YOUR ANSWER 
    $\frac{\partial \ell}{\partial z} = \beta^* \cdot \left(\frac{\partial \ell}{\partial b}\right)$
    \end{your_solution}
    ...
    % YOUR ANSWER 
    $\partial \ell/\partial a_j = \frac{\partial \ell}{\partial z_j}z_j(1-z_j)$
    \end{your_solution}
    ...
    % YOUR ANSWER 
    $\partial \ell/\partial \alphav = \frac{\partial \ell}{\partial \av}(\xv^{(1)})^T$
    \end{your_solution}
...
% YOUR ANSWER 
As the number of hidden layers increases, the model becomes more complex, and the training cross-entropy loss continues to decrease. However, the validation cross-entropy loss reaches its lowest point at 100 hidden units, and then increases at 200 hidden units, indicating that the model overfits the training data. This overfitting leads to poor performance on unseen data.
\end{your_solution}
...
% YOUR ANSWER 
A learning rate of 0.03 is too large, resulting in overfitting and poor validation loss.

A learning rate of 0.003 seems to be the optimal point where the training loss and validation loss largely converge.

A learning rate of 0.0003 is not fast enough to converge, and the training process may be slow and inefficient.
\end{your_solution}
...
\begin{your_solution}[title=Answer,height=4 cm,width=14.5cm]
% YOUR ANSWER 
Based on the results, it appears that a learning rate of 0.03 is too large, resulting in overfitting and poor validation loss. The model is training too quickly and is not generalizing well to new data.

A learning rate of 0.003 seems to be the optimal point where the training loss and validation loss largely converge, with some overfitting occurring but not as extreme as with a higher learning rate of 0.03.
\end{your_solution}
...
\begin{your_solution}[title=Answer,height=5cm,width=14.5cm]
% YOUR ANSWER 
Zero initialization is not a suitable choice because it can lead to a neural network where all neurons follow the same path, resulting in a relatively low model capacity and poorer convergence. This is due to the fact that the identical weights and biases lead to symmetric networks, and each neuron in a layer behaves in the same way, making it difficult for the network to capture the complex patterns and relationships in the data. 
\end{your_solution}
