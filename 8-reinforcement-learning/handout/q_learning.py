...
def Q(W: np.ndarray, state: np.ndarray,
    ...
    qa_vals = W @ state_concat
    if action is not None:
        return qa_vals[action]
    return qa_vals
...
if __name__ == "__main__":
    ...
    W = np.zeros((env.action_space, env.state_space + 1))  # initialize the weights with all zeros (including bias)
    ...
    for episode in range(episodes):
        ...
        for iteration in range(max_iterations):
            if random() <= epsilon:
                action = randint(0, env.action_space - 1)
            else:
            ...
            # update the weights
            target = reward + gamma * np.max(Q(W, next_state))
            td_error = target - Q(W, state, action)
            W[action] += lr * td_error * np.concatenate(([1.0], state))
